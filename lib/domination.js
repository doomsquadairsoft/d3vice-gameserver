 ...
 * heartbeat
 * 
 * generate and emit a heartbeat event
 */
Game.prototype.heartbeat = function heartbeat() {
    var self = this;
    console.log('timer_red=%s, timer_green=%s', self.timers[0], self.timers[1]);
    self.emit('heartbeat', {
	isRunning: self.isRunning,
	startTime: self.startTime,
	lastTickTIme: self.lastTickTime,
	teams: self.teams,
	controlTeam: self.controlTeam,
	timeToWin: self.timeToWin,
	timers: self.timers
    });
}
...
Game.prototype.run = function run() {
	...
	// emit a heartbeat event every 1s
 ...
 * hookInput
 *
 * Hook used by external libraries to tell the game instance that sensor input has occured.
 * it is up to the game to interpret the sensor data and act accordingly
 *
 * @param {object} data - the data received from the sensor
 * @param {string} data.id - the id number of the sensor
 * @param {string} [data.button] - the name of the button pressed
 */
Game.prototype.hookInput = function hookInput(data) {
    var self = this;
    
    assert.isObject(data, 'the data object passed to hookInput was not an object.');
    
    var id = data.id || 'null';
    var button = data.button || 'null';

    // using the button, set the control team
    if (button === 'green')
	self.setControlTeam(1);

    if (button === 'red')
	self.setControlTeam(0);
 ...
 * setControlTeam
 *
 * @param {number} team - the team number who has gained control of the point
 */
