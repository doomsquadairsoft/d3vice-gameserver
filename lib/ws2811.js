...
var debug = require('debug')('d3vice-gameserver:ws2811');


console.log('||| DEBUG ENV VAR IS %s |||', process.env.DEBUG);

var Display = function Display(opts) {
    debug('>>> init ws2811 lib');
...
Display.prototype.setColor = function setColor(color, percentage) {
    ...
    debug('setColor() got {%s} %s and {%s} %s', typeof color, color, typeof percentage, percentage);
    if (typeof color !== 'string') throw new Error('setColor must have a {string} colorname as first param');
    if (typeof percentage !== 'number') {
	debug('no {number} percentage sent to setColor(). using cache instead.');
	if (typeof self.colorCache[color] !== 'number') {
	    debug('no colorCache for %s. using default instead.', color);
	    percentage = 50;
	}
	else {
	    percentage = self.colorCache[color];
	}
    }
    percentage = Math.ceil(percentage);
    self.colorCache[color] = percentage;
	...
	debug('value=%s, fromLow=%s, fromHigh=%s, toLow=%s, toHigh=%s', value, fromLow, fromHigh, toLow, toHigh);
	var percentage = value / fromHigh;
	return mappedValue = Math.ceil(percentage * toHigh);
    }

    // example
    //debug('mapped 80. return=%s', map(80, 0, 100, 0, 4));
	...
	if (count < mapped) {
	    debug('section 1 pixel %s', i);
	    self.pixelData[i] = rgb2Int(redValue, grnValue, bluValue);
	}
	else {
	    self.pixelData[i] = rgb2Int(0, 0, 0);
	}
	count++;
    }
